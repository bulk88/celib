...
xceshared_setenvironblock(char *env)
{
  ...
  endp = psdata->procstart.environ + MAX_ENVIRONBLOCKSIZE;
  ...
  if(pipe_getavail(pp) == 0)
    {
      ...
      XCETrace("pipe_reader: signalling writer, avail = %d",
	       pipe_getavail(pp));
...
close_pipe(HANDLE handle)
{
  ...
  XCETrace("close_pipe(0x%x): openread %d/openwrite %d",
	   handle,
...
pipe_increment_opencount(HANDLE hFile)
{
  ...
  XCETrace("inc_open_count(0x%x): before: read %d/write %d",
	   hFile,
	   psdata->pipetab[pipeindex].opencntread,
	   psdata->pipetab[pipeindex].opencntwrite);
  ...
  XCETrace("inc_open_count(0x%x): after: read %d/ write %d",
	   hFile,
	   psdata->pipetab[pipeindex].opencntread,
	   psdata->pipetab[pipeindex].opencntwrite);
		 ...
		 LPDWORD  lpcbMessage)
{
  ...
  if(pp->used == 0 || pp->opencntread == 0)
    {
      XCETrace("peek on pipe with closed read handle");
      errno = EPIPE;
      SetLastError(ERROR_BROKEN_PIPE);
      return FALSE;
    }
  if(pp->used == 0 || pp->opencntwrite == 0)
    {
      XCETrace("peek on pipe with closed write handle");
      errno = EPIPE;
      SetLastError(ERROR_BROKEN_PIPE);
      return FALSE;
    }
